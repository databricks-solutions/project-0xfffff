...
|------|-------|-------------|
| Unit | 715 | pytest unit tests, Vitest tests |
| Integration | 50 | pytest with real DB/API |
| E2E (Mocked) | 20 | Playwright with mocked API |
| E2E (Real) | 42 | Playwright with real API |
...
| [DISCOVERY_SPEC](#discovery-spec) | 50 | 38 | 76% | 177 | 4 | 7 | 7 |
...
**Total**: 155/249 requirements covered (62%)
...
**Coverage**: 38/50 requirements (76%)
...
- [x] Completion status shows % of participants finished (integration, unit)
...
- [x] System aggregates feedback by trace (unit)
- [x] Disagreements detected at 3 priority levels (deterministic, no LLM) (unit)
- [x] LLM distills evaluation criteria with evidence from trace IDs (unit)
- [x] LLM analyzes disagreements with follow-up questions and suggestions (unit)
- [x] Analysis record stores which template was used (unit)
- [x] Each analysis run creates a new record (history preserved) (e2e-mocked, unit)
- [x] Re-runnable — new analysis as more feedback comes in, prior analyses retained (unit)
...
- [x] One feedback record per (workshop, trace, user) — upsert behavior (integration, unit)
- [x] Q&A pairs appended in order to JSON array (integration, unit)
- [x] Multiple analysis records per workshop allowed (history preserved) (unit)
...
- [x] Disagreements color-coded by priority (red/yellow/blue) (unit)
- [x] Criteria show evidence (supporting trace IDs) (unit)
