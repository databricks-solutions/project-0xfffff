...
// @req Authenticated facilitators with `can_manage_project` can access `/project/setup` when no project has completed setup
...
describe('ProjectSetupGate', () => {
  ...
  it('keeps non-managers away from the setup form', async () => {
    userContext.value = {
      ...
      permissions: { can_manage_project: false },
    };
    vi.spyOn(globalThis, 'fetch').mockResolvedValue(
      new Response(
        JSON.stringify({ detail: 'Setup job not found' }),
