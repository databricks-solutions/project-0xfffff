...
async def get_discovery_questions(
    ...
    """Get discovery questions for the participant UI.

    Returns per-user/per-trace questions. If a real model is configured for the workshop,
    generates questions via Databricks model serving; otherwise falls back to a fixed question.
    """
    db_service = DatabaseService(db)
    workshop = db_service.get_workshop(workshop_id)
    if not workshop:
    ...
    trace = db_service.get_trace(trace_id)
    ...
    fixed_question = DiscoveryQuestion(
        id="q_1",
    ...
    if not user_id:
        # We can't persist per-user questions without a user_id, but we still always ask the fixed question.
        return DiscoveryQuestionsResponse(questions=[fixed_question])

    # Load existing generated questions for this (user, trace).
    existing_questions_raw = db_service.get_discovery_questions(workshop_id, trace_id, user_id)
    existing_questions: list[DiscoveryQuestion] = [
        DiscoveryQuestion(
            id=str(q.get("id")),
    ...
    existing_questions = [q for q in existing_questions if q.id != fixed_question.id]

    model_name = (getattr(workshop, "discovery_questions_model_name", None) or "demo").strip()
    if not model_name or model_name == "demo":
        # No LLM configured; return existing if any, otherwise fixed.
        return DiscoveryQuestionsResponse(questions=[fixed_question, *existing_questions])
    ...
    # We use the user's own prior response for this trace (if any), plus other users' findings for context.
    # This data is NOT returned to the client; it is only used to generate a better question.
    user_prior_finding_text = ""
    other_findings_texts: list[str] = []
    try:
        user_findings = db_service.get_findings(workshop_id, user_id=user_id)
        ...
        all_findings = db_service.get_findings(workshop_id)
        trace_findings = [f for f in all_findings if f.trace_id == trace_id and f.user_id != user_id]
        other_findings = trace_findings
        # Limit to a small number of other-user findings to avoid prompt bloat.
    ...
    if not databricks_token:
        logger.warning("Discovery question generation requested but Databricks token missing; falling back to fixed.")
        return DiscoveryQuestionsResponse(questions=[fixed_question, *existing_questions])

    # Call Databricks model serving using structured outputs (JSON schema).
    # See: https://docs.databricks.com/aws/en/machine-learning/model-serving/structured-outputs
    try:
        from server.services.databricks_service import DatabricksService

        svc = DatabricksService(workspace_url=mlflow_config.databricks_host, token=databricks_token)

        def _parse_llm_json_message(message: Any) -> dict:
            """Parse model output expected to be JSON.

            Primary path: message.content is already a dict, or a JSON string.
            Alternate path: message.tool_calls[].function.arguments contains JSON.
            Fallback path: if structured outputs were rejected and we retried without them,
            the model may return text that *contains* a JSON object; in that case, extract
            the outer-most JSON object.
            """
            ...
            if isinstance(content, list):
                # Some clients/models may return content as a list of parts
                parts: list[str] = []
                for p in content:
                    if isinstance(p, str):
                        parts.append(p)
        ...
        trace_context = json.dumps(trace.context, ensure_ascii=False) if trace.context is not None else ""
        other_findings_block = "\n".join(
            f"- {_trim(txt, 600)}" for txt in other_findings_texts if txt and _trim(txt, 600)
        )

        existing_prompts_block = "\n".join(f"- {q.prompt}" for q in existing_questions if q.prompt)

        prompt = f"""You are generating discovery questions for an LLM evaluation workshop.

Goal: ask ONE question that will help elicit **themes and novel insights** from the user about the model output.
The question should help the user contribute something new (not just repeat what's already been said).

You may use:
- The user's prior answer for this trace (if any)
- Other users' findings for this trace (if any)

Constraints:
- The question must be a single concise prompt (1-2 sentences).
- Avoid asking for a generic summary.
- Encourage comparison, edge cases, failure modes, missing info, and root causes.
- Do NOT quote other users' findings verbatim.
- Avoid repeating questions that have already been asked for this user/trace.
...
Trace Input:
{_trim(trace.input or "", 2000)}

Trace Output:
{_trim(trace.output or "", 2000)}

Context (optional, JSON):
{_trim(trace_context, 2000)}

User's prior finding for this trace (may be empty):
{_trim(user_prior_finding_text, 1200)}

Previously asked questions for this user/trace (may be empty):
{existing_prompts_block or "(none)"}

Other users' findings for this trace (may be empty):
{other_findings_block or "(none)"}
"""
        ...
        result = svc.call_serving_endpoint(
        ...
        message = ((result or {}).get("choices") or [{}])[0].get("message") or {}
        payload = _parse_llm_json_message(message)

        questions = payload.get("questions")
        if not isinstance(questions, list) or not questions:
            raise ValueError("JSON missing questions[]")

        normalized: List[DiscoveryQuestion] = []
        for i, q in enumerate(questions):
            if not isinstance(q, dict):
                continue
            qprompt = str(q.get("prompt") or "").strip()
            if not qprompt:
                continue
            normalized.append(
                DiscoveryQuestion(
                    id=str(q.get("id") or f"gen_{i + 1}"),
                    prompt=qprompt,
                    placeholder=(str(q.get("placeholder")).strip() if q.get("placeholder") is not None else None),
                )
            )
        ...
        # Append ONLY the first generated question, persist it, and return the full list.
        generated = normalized[0]
        created = db_service.add_discovery_question(
        ...
        existing_questions.append(
            DiscoveryQuestion(
                ...
                placeholder=(str(created["placeholder"]).strip() if created.get("placeholder") is not None else None),
            )
        )
        return DiscoveryQuestionsResponse(questions=[fixed_question, *existing_questions])

    except Exception as e:
        logger.exception("Failed to generate discovery questions; falling back to fixed: %s", e)
        return DiscoveryQuestionsResponse(questions=[fixed_question, *existing_questions])
...
async def update_discovery_questions_model(
    ...
    """Set the model/endpoint used to generate discovery questions for this workshop."""
    db_service = DatabaseService(db)
    workshop = db_service.get_workshop(workshop_id)
    if not workshop:
        raise HTTPException(status_code=404, detail="Workshop not found")

    updated = db_service.update_discovery_questions_model_name(workshop_id, config.model_name)
    if not updated:
        raise HTTPException(status_code=500, detail="Failed to update discovery questions model")

    return {"message": "Discovery questions model updated", "model_name": updated.discovery_questions_model_name}
...
async def generate_discovery_summaries(
    ...
    """Generate facilitator-oriented discovery summaries using an LLM.

    Produces:
    - overall themes/patterns/tendencies
    - grouped by user
    - grouped by trace_id
    """
    db_service = DatabaseService(db)
    ...
    # Return persisted summary unless caller explicitly requests refresh.
    if not refresh:
        cached = db_service.get_latest_discovery_summary(workshop_id)
    ...
    findings = db_service.get_findings_with_user_details(workshop_id)
    if not findings:
        return DiscoverySummariesResponse(overall={"themes": []}, by_user=[], by_trace=[])
    ...
    for f in findings[:300]:  # hard cap
        corpus_lines.append(
            f"TRACE {f.get('trace_id')} | USER {f.get('user_name')} ({f.get('user_id')}): {_trim(f.get('insight') or '', 800)}"
        )
    ...
    try:
        ...
        prompt = f"""You are helping a workshop facilitator understand discovery findings about an LLM's behavior.

Given the findings below, summarize:
1) OVERALL themes, patterns, and tendencies of the model's behaviors.
2) BY USER: what each user tends to focus on (themes/tendencies) and any novel angles.
3) BY TRACE: what behaviors/issues/themes appear for each trace_id.

Rules:
- Focus on themes/patterns/tendencies of the MODEL behavior (not participant performance).
- Avoid quoting participants verbatim; paraphrase.
- Be specific: mention concrete behavior categories (e.g., hallucination, instruction following, verbosity, refusal, safety, formatting, reasoning transparency, tool use).
- Note disagreements or divergent viewpoints.
- Keep each bullet short.

Findings (each line is one submission):
...
"""
                    ...
                    "properties": {
                        "overall": {
                            ...
                            "properties": {
                                ...
                                "risks_or_failure_modes": {"type": "array", "items": {"type": "string"}},
                                "strengths": {"type": "array", "items": {"type": "string"}},
                            },
                            "required": ["themes", "patterns", "tendencies", "risks_or_failure_modes", "strengths"],
                        },
                        "by_user": {
                            "type": "array",
                            "items": {
                                ...
                                "properties": {
                                    ...
                                    "themes": {"type": "array", "items": {"type": "string"}},
                                    "tendencies": {"type": "array", "items": {"type": "string"}},
                                    "notable_insights": {"type": "array", "items": {"type": "string"}},
                                },
                                "required": ["user_id", "user_name", "themes", "tendencies", "notable_insights"],
                            },
                        },
                        "by_trace": {
                            "type": "array",
                            "items": {
                                ...
                                "properties": {
                                    ...
                                    "themes": {"type": "array", "items": {"type": "string"}},
                                    "tendencies": {"type": "array", "items": {"type": "string"}},
                                    "notable_behaviors": {"type": "array", "items": {"type": "string"}},
                                },
                                "required": ["trace_id", "themes", "tendencies", "notable_behaviors"],
                            },
                        },
                    },
                    "required": ["overall", "by_user", "by_trace"],
                },
                "strict": True,
            },
        }
        ...
        # Persist for facilitator reuse (latest wins).
        try:
            db_service.save_discovery_summary(workshop_id=workshop_id, payload=payload, model_name=model_name)
        ...
        logger.exception("Failed to generate discovery summaries: %s", e)
        raise HTTPException(status_code=502, detail=f"Failed to generate summaries: {str(e)}")
...
async def get_discovery_summaries(workshop_id: str, db: Session = Depends(get_db)) -> DiscoverySummariesResponse:
    """Fetch the most recently persisted discovery summaries for a workshop."""
    db_service = DatabaseService(db)
    workshop = db_service.get_workshop(workshop_id)
    if not workshop:
        raise HTTPException(status_code=404, detail="Workshop not found")
                    ...
                    if rubric.question:
                        ...
                        if questions:
                            ...
                            if binary_questions and not likert_questions:
                                ...
                                job.add_log(
                                    f"⚠️ No judge_type found in questions - will fall back to rubric-level judge_type"
                                )
