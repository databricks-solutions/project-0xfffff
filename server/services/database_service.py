...
class DatabaseService:
    ...
    def get_workshop(self, workshop_id: str) -> Optional[Workshop]:
        ...
        workshop = Workshop(
            ...
            discovery_questions_model_name=getattr(db_workshop, "discovery_questions_model_name", None) or "demo",
            created_at=db_workshop.created_at,
        )
    ...
    def update_discovery_questions_model_name(self, workshop_id: str, model_name: str) -> Optional[Workshop]:
        """Update the model/endpoint used to generate discovery questions for a workshop."""
        db_workshop = self.db.query(WorkshopDB).filter(WorkshopDB.id == workshop_id).first()
        if not db_workshop:
            return None
    ...
    # Discovery question operations
    def get_discovery_questions(self, workshop_id: str, trace_id: str, user_id: str) -> List[Dict[str, Any]]:
        """Get generated discovery questions for a specific (workshop, trace, user)."""
        rows = (
    ...
    def add_discovery_question(
        ...
        # Generated questions always start at q_2.
        existing_ids = (
            self.db.query(DiscoveryQuestionDB.question_id)
            .filter(
                DiscoveryQuestionDB.workshop_id == workshop_id,
                DiscoveryQuestionDB.trace_id == trace_id,
                DiscoveryQuestionDB.user_id == user_id,
        ...
        for (qid,) in existing_ids:
            if isinstance(qid, str) and qid.startswith("q_"):
                try:
                    n = int(qid.split("_", 1)[1])
                    if n > max_n:
                        max_n = n
                except Exception:
                    continue
        question_id = f"q_{max_n + 1}"

        row = DiscoveryQuestionDB(
            workshop_id=workshop_id,
            trace_id=trace_id,
            user_id=user_id,
    ...
    def get_latest_discovery_summary(self, workshop_id: str) -> Optional[Dict[str, Any]]:
        """Get the most recent persisted discovery summary payload for a workshop."""
        row = (
    ...
    def save_discovery_summary(
        self, workshop_id: str, payload: Dict[str, Any], model_name: Optional[str] = None
    ) -> Dict[str, Any]:
        """Persist a discovery summary payload for a workshop."""
        row = DiscoverySummaryDB(
            workshop_id=workshop_id,
    ...
    def update_rubric_question(
        self, workshop_id: str, question_id: str, title: str, description: str, judge_type: Optional[str] = None
    ) -> Optional[Rubric]:
    ...
    def get_annotations_with_user_details(
    ...
    def update_mlflow_ingestion_status(
        self, workshop_id: str, trace_count: int, error_message: Optional[str] = None
    ) -> None:
            ...
            if validated_predicted_rating is not None:
                if is_binary:
                    ...
                    if validated_predicted_rating == 0:
                        ...
                        logger.error(
                            f"Invalid binary predicted_rating {original_value} for trace {evaluation.trace_id[:8]}... - must be 0 or 1, rejecting evaluation"
                        )
                else:
                    ...
                    if not (1 <= validated_predicted_rating <= 5):
                        ...
                        logger.warning(
                            f"Likert predicted_rating {original_value} out of range for trace {evaluation.trace_id[:8]}... - clamped to {validated_predicted_rating}"
                        )
