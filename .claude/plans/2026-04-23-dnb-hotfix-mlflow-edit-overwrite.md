...
1. **`update_assessment` timestamps**: the MLflow docs say mutation happens in place. Whether `create_time_ms` is updated or preserved isn't explicit — needs to be verified against a real MLflow instance during `dnb-jbw` testing. Either way, MemAlign's "most recent wins" rule still picks the updated entry.
